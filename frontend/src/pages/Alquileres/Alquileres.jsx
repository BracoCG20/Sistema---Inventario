...
const Alquileres = () => {
  ...
  const equiposOcupadosIds = alquileres
    .filter((alq) => {
      if (alq.estado !== 'Activo') return false;
      if (!alq.fecha_fin) return true;
  ...
  // --- HANDLERS CORREGIDOS (Se quitó el header manual para que Axios asigne el Boundary) ---
  const handleCreate = async (data, file) => {
    try {
      ...
      // IMPORTANTE: NO enviamos el header 'Content-Type' manualmente
      await api.post('/alquileres', formPayload);
  ...
  const handleUpdate = async (data, file, fileDeleted) => {
    try {
      const formPayload = new FormData();
      Object.keys(data).forEach((key) => {
        formPayload.append(key, data[key]);
      });
      if (file) formPayload.append('archivo', file);

      // IMPORTANTE: NO enviamos el header 'Content-Type' manualmente
      await api.put(`/alquileres/${editItem.id}`, formPayload);

      if (fileDeleted && !file) {
        await api.delete(`/alquileres/${editItem.id}/eliminar-factura`);
      }
  ...
  return (
    <div className='alquileres-container'>
      <div className='page-header'>
        <h1>
          <FaHandHoldingUsd style={{ marginRight: '10px' }} /> Alquiler de
          Equipos
        </h1>
        ...
        {selectedAlquiler && (
          <div className='preview-container'>
            ...
            <div className='preview-stack'>
              ...
              {/* --- NUEVA TARJETA DE OBSERVACIONES --- */}
              {selectedAlquiler.observaciones && (
                <div className='preview-card'>
                  <h5>
                    <FaAlignLeft /> Observaciones del Contrato
                  </h5>
                  <div
                    style={{
                      marginTop: '10px',
